...
\
def _normal_description(text: str) -> str:
    text = (text or "").lower().strip()

    replacements = {
        "payroll credit": "payroll",
        "next confirmed salary": "payroll",
        "final employer payroll": "payroll",
        "international employer payroll": "payroll",
...
def _stream_key(event: FinancialEvent) -> tuple:
    return (
        ...
        event.category,
        event.direction,
        _normal_description(event.description),
    )
...
def _compatible_future_event(
    ...
    for event in explicit_events:
        ...
        if event.category != template.category:
            continue

        if event.direction != template.direction:
            continue

        event_date = _event_date(event)

        if abs((event_date - projected_date).days) > tolerance_days:
            continue

        # A confirmed future event should be similar in scale to the
        # recurring stream it replaces. This prevents an unrelated
        # exceptional transaction in the same category from suppressing
        # a normal recurrence.
        denominator = max(abs(projected_amount), Decimal("0.01"))
        relative_difference = (
            abs(event.amount - projected_amount) / denominator
        )

        if relative_difference <= Decimal("0.15"):
            return True
...
def _recurring_flows(
    ...
    for key, group in groups.items():
        ...
        while True:
            ...
            if _compatible_future_event(
                template=template,
                projected_date=cursor,
